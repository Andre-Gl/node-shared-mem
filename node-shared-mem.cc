...
SharedMemory::SharedMemory(const Napi::CallbackInfo& info) : Napi::ObjectWrap<SharedMemory>(info)
{
	...
	if (info.Length() < 3)		fail("needs mapName, mapSize, create");
	...
	if (!info[2].IsBoolean())	failt("argument 2 needs to be a valid create flag");
	...
	void* data = mmap(NULL, len, PROT_READ | PROT_WRITE, MAP_SHARED, key, 0);
	if(data == MAP_FAILED) {
